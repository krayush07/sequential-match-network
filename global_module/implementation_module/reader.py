...
def get_index_string(utt, word_dict):
    ...
    for each_token in utt.split():
        if (set_params.ParamsClass('TR').all_lowercase):
...
def generate_id_map(params, data_filename, label_filename, index_arr, dict_obj):
    ...
    for curr_id, each_idx in enumerate(index_arr):
        ...
        curr_ctx_seq_arr = []
        curr_ctx_len_arr = [0 for _ in range(params.NUM_CONTEXT)]

        for idx in range(len(data_line_split) - 1):
            each_split = data_line_split[idx]
            curr_string_len, curr_index_string = get_index_string(each_split, dict_obj.word_dict)
            curr_utt_string = format_string(curr_index_string, curr_string_len, params.MAX_CTX_UTT_LENGTH)  # format each utt string
            curr_ctx_seq_arr.append(curr_utt_string)
            curr_ctx_len_arr[idx] = curr_string_len

        curr_ctx_seq_arr = add_dummy_context_string(curr_ctx_seq_arr, curr_num_context, params.NUM_CONTEXT, params.MAX_CTX_UTT_LENGTH)

        resp_string_len, resp_index_string = get_index_string(data_line_split[curr_num_context], dict_obj.word_dict)
        resp_index_string = format_string(resp_index_string, resp_string_len, params.MAX_RESP_UTT_LENGTH)  # format resp string
...
def data_iterator(params, data_filename, label_filename, index_arr, dict_obj):
    ctx_arr, ctx_len_arr, num_ctx_arr, resp_arr, resp_len_arr, label_arr = generate_id_map(params, data_filename, label_filename, index_arr, dict_obj)
...
def main():
    ...
    a, b, c, d, e, f = data_iterator(config, data_file, label_file, utterance_idx, dictObj)
